class Event < ActiveRecord::Base
  ...
  def self.search(value)
    if value
      hits_city = []
      hits_city += find(:all, :conditions => ['city like ?', '%' + value + '%'])
      if hits_city.empty?
        hits_city += find(:all, :conditions => ['city like ?', '%' + value.downcase + '%'])
      end
      
      hits_title = []
      hits_title += find(:all, :conditions => ['title like ?', '%' + value + '%'])
      if hits_title.empty?
        hits_title += find(:all, :conditions => ['title like ?', '%' + value.downcase + '%'])
      end
      
      hits_description = []
      hits_description += find(:all, :conditions => ['description like ?', '%' + value + '%'])
      if hits_description.empty?
        hits_description += find(:all, :conditions => ['description like ?', '%' + value.downcase + '%'])
      end
